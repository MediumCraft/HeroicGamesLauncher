 ...
 */
function setupEnvVars(gameConfig: GameConfig) {
...
function setupWineEnvVars(
  ...
  if (gameConfig.fSync && wineVersion.type !== 'proton') {
    ret.WINEFSYNC = '1'
  }
  if (!gameConfig.fSync && wineVersion.type === 'proton') {
    ret.PROTON_NO_FSYNC = '1'
  }
  if (gameConfig.autoInstallDxvkNvapi && wineVersion.type === 'proton') {
    ret.PROTON_ENABLE_NVAPI = '1'
    ret.DXVK_NVAPI_ALLOW_OTHER_DRIVERS = '1'
  }
  if (gameConfig.autoInstallDxvkNvapi && wineVersion.type === 'wine') {
    ret.DXVK_ENABLE_NVAPI = '1'
    ret.DXVK_NVAPI_ALLOW_OTHER_DRIVERS = '1'
  }
  if (gameConfig.eacRuntime) {
    ret.PROTON_EAC_RUNTIME = join(runtimePath, 'eac_runtime')
  }
  if (gameConfig.battlEyeRuntime) {
...
async function runWineCommand({
  ...
  const env_vars = {
    ...process.env,
    ...setupEnvVars(gameConfig),
    ...setupWineEnvVars(gameConfig, installFolderName, gameInstallPath)
  }
